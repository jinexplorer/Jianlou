...
public class PublishGoodActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void init() {
        ...
        progressBar=findViewById(R.id.publish_bar);
    ...
    private void postData() {
        ...
        params.put(Table.content, content.getText().toString());
        params.put(Table.money, m1);
        params.put(Table.origin_money, m2);
        params.put(Table.send_money, m3);
        params.put(Table.classify, classify.getText().toString());
        ...
        if (fileList != null) {
            for (File f : fileList) {
                bodyBuilder.addFormDataPart("image", f.getName(),
                        RequestBody.create(MediaType.parse("image/png"), f));
            }
        }

        HttpUtil.sendOkHttpRequest(StaticVar.publishUrl, bodyBuilder.build(), new okhttp3.Callback() {
            @Override
